...
const ImageGallery = () => {
  ...
  return (
    <div className="container mx-auto bg-base-100 rounded-lg shadow-2xl p-4 md:p-10 m-2 w-full">
      {showGalleryText && (
      ...
      <div className="flex flex-col md:flex-row justify-between border-spacing-x-5 items-center mb-2 md:mb-4">
        ...
        {selectedItems.length > 0 && (
          <button
            onClick={onDelete}
            className="text-red-600 hover:underline font-bold mt-2 md:mt-0"
          >
            Delete Files
          </button>
        )}
      ...
      <SortableList
        onSortEnd={onSortEnd}
        draggedItemClassName="p-4 cursor-pointer border"
      >
          ...
          <div className="grid lg:grid-cols-1 gap-2">
            {items
              ...
              .map(({ id, image, selected }) => (
                <SortableItem key={id}>
                  <div className={`m-2 cursor-pointer big-image lg:ml-10`}>
